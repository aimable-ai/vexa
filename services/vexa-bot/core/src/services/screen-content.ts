  ...
  private async _drawAvatarOnCanvas(avatarUri: string): Promise<void> {
    await this.page.evaluate(async (imgSrc: string) => {
      ...
      return new Promise<void>((resolve) => {
        ...
        img.crossOrigin = 'anonymous';
        img.onload = () => {
          // Black background fills the whole canvas
          ctx.fillStyle = '#000000';
          ctx.fillRect(0, 0, canvas.width, canvas.height);

          // Draw a centered white rounded-rect "card" — gives the logo a
          // clean frame and works for any logo (transparent SVG/PNG against
          // black would otherwise look lost). AIM-951 branding.
          const cardSize = Math.round(Math.min(canvas.width, canvas.height) * 0.28);
          ...
          // Logo sits inside the card with ~14% padding all around.
          const pad = Math.round(cardSize * 0.14);
