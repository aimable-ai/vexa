...
export function createOrchestrator(inv: Invocation, deps: OrchestratorDeps) {
  ...
  async function run(opts: RunOptions = {}): Promise<MeetingResult> {
    ...
    if (crashText) {
      await emit('failed', { failure_stage: 'active', completion_reason: 'join_failure', reason: crashText, exit_code: 1, speaker_events: deps.pipeline.speakerEvents?.(), participants: deps.pipeline.participants?.() });
      return { exitCode: 1, status: 'failed', completionReason: 'join_failure' };
    }
    console.error(`[bot] orchestrator: emitting completed (reason=${reason}, from=${cur})`);
    try {
      await emit('completed', { completion_reason: reason, exit_code: 0, speaker_events: deps.pipeline.speakerEvents?.(), participants: deps.pipeline.participants?.() });
